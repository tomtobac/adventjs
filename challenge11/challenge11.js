...
export default function shouldBuyFidelity(times) {
  ...
  const discount = 0.75;
  const priceWithoutDiscount = normalPrice * times;
  let priceWithFidelityCard = fidelityCard;
  for (let index = 1; index <= times; index++) {
    const actualDiscount = discount ** index;
    priceWithFidelityCard += normalPrice * actualDiscount;
  }
  return priceWithFidelityCard <= priceWithoutDiscount;
}
