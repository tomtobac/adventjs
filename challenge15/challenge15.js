...
*/
const isNotUndefined = (n) => n !== undefined;

export default function checkSledJump(heights) {
  const max = Math.max(...heights);
  const idx = heights.findIndex((h) => h === max);
  const goingUp = heights.slice(0, idx + 1);
  const goingDown = heights.slice(idx, -1);

  const isGoingUp = goingUp.every((h, i, a) => (isNotUndefined(a[i + 1]) ? h < a[i + 1] : true));
  const isGoingDown =
    !!goingDown.length &&
    goingDown.every((h, i, a) => (isNotUndefined(a[i + 1]) ? h > a[i + 1] : true));
