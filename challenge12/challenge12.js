...
getMinJump(obstacles) // -> 4 // Fixed (9)
...
avoidObstacles([3, 7, 5]) // -> 3 // Fixed (2)
...
export default function getMinJump(obstacles) {
  ...
  for (let jumpSize = 1; jumpSize <= highestObstacle; jumpSize++) {
    for (let i = 0; i <= highestObstacle; i = i + jumpSize) {
      // It's not in the obastacles list
      if (sortedObstacles.includes(i)) {
        break;
      }

      // if in the next iteration we are out, it's a good jump
      const nextPos = i + jumpSize;
      if (nextPos > highestObstacle && jumpSize < jump) {
        jump = jumpSize;
      }
    }
  }
